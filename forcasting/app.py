...
# start_query = st.text_input("Search Start Location in Hyderabad", "")
# if start_query:
#     start_options = search_location(start_query)
#     start_labels = [f"{feat['properties']['name']}, {feat['properties'].get('locality', 'Hyderabad')}" for feat in start_options]
#     if start_labels:
#         start_choice = st.selectbox("Select Start Location", start_labels)
#         if start_choice:
#             selected_start = start_options[start_labels.index(start_choice)]
#             st.session_state.start = (selected_start['geometry']['coordinates'][1], selected_start['geometry']['coordinates'][0])
#             st.write(f"✅ Start: {start_choice} ({st.session_state.start})")
# destination_query = st.text_input("Search Destination Location in Hyderabad", "")
# if destination_query:
#     dest_options = search_location(destination_query)
#     dest_labels = [f"{feat['properties']['name']}, {feat['properties'].get('locality', 'Hyderabad')}" for feat in dest_options]
#     if dest_labels:
#         dest_choice = st.selectbox("Select Destination Location", dest_labels)
#         if dest_choice:
#             selected_dest = dest_options[dest_labels.index(dest_choice)]
#             st.session_state.destination = (selected_dest['geometry']['coordinates'][1], selected_dest['geometry']['coordinates'][0])
#             st.write(f"✅ Destination: {dest_choice} ({st.session_state.destination})")
...
if st.session_state.graph_data is not None and not st.session_state.graph_data.empty:
    ...
    # heatmap_data = st.session_state.input_data_df.groupby("hour_of_day")["traffic_density"].mean().reset_index()
    # fig2 = px.density_heatmap(heatmap_data, x="hour_of_day", y="traffic_density", title="Traffic Density by Hour")
    # st.plotly_chart(fig2)

    # st.write("### 🌧️ Weather vs Traffic Density")
    # fig3 = px.bar(st.session_state.input_data_df, x="weather", y="traffic_density", color="weather", title="Impact of Weather on Traffic")
    # st.plotly_chart(fig3)

    # st.write("### 📉 Rolling Average of Traffic Density")
    # st.session_state.input_data_df["rolling_avg"] = st.session_state.input_data_df["traffic_density"].rolling(window=2).mean()
    # fig4 = px.line(st.session_state.input_data_df, x="timestamp", y="rolling_avg", markers=True, title="Rolling Average of Traffic Density")
